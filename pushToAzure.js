...
const buildName = moment(new Date()).subtract(3, 'hours').format("DD-MM-YYYYTHH-mm-ss") + ".zip"
...
async function pushFilesToAzure() {
    const blobServiceClient = BlobServiceClient.fromConnectionString(azureConnection.AZURE_CONNECTION);
    const containerClient = blobServiceClient.getContainerClient(azureConnection.AZURE_BUCKET);
    ...
    if (response._response.status !== 201) {
        console.log(`Error uploading document ${blockBlobClient.name} to container ${blockBlobClient.containerName}`)
    }
}
